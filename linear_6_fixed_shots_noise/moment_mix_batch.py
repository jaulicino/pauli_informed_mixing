...
n_qubits = 6# int(input())
n_moments_exact = 5#int(input())
K = 6
...
circuits_per_job = 20
theta_test = np.genfromtxt(save_dir+"/theta_"+str(n_qubits)+"_save.txt") # get the ansatz rotation
psi_numerical = circuit_numerical(theta_test)
CUT_OFF = 5 # m < CUT_OFF -> QWC, note not inclusive 
ham_powers = []
NPROCS = 8 # how many cores does the simulator use
qwc_shots = 2536 # how many shots for each QWC base
trial_ks = [1,2,3,4,5,6]
...
print("A total of ", len(total_paulis), " to be measured ")
print("------------------------------------")
...
for round_index in range(n_rounds): # run several experiments for each method
    ...
    with open("pure_qwc_expectations/"+save_dir+"_"+str(round_index)+".json", "w") as fp:
    ...
    qbackend.set_options(max_job_size=NPROCS)
    qbackend.set_options(noise_model=noise_model)
    job = qm.execute(circs, shots=qwc_shots, backend=qbackend)
    mix_qwc_expectations = qwc_expectation_from_qiskit_job(job, mix_qwc_bases, qwc_shots, mix_qwc_paulis)
    print(len(mix_qwc_expectations))
    ...
    print("N Commuting Cliffords ", len(commuting_cliffords))     
    circuits = []
    for unitary in commuting_cliffords:
        ...
        for i, l in enumerate(unitary):
            ...
            qc.measure(i, i)
        circuits += [qc]
    shots_ = int((budget - qwc_shots * len(mix_qwc_bases))/len(commuting_cliffords))
    ...
    data_save["mix_shots"] = qwc_shots * len(mix_qwc_bases) + snap.shape[0]
    data_save["mix_moments"] = [str(s) for s in list(mix_moments.values())]
    with open("mix_expectations/"+save_dir+"_"+str(round_index)+".json","w") as fp:
    ...
    with open("pure_cs_expectations/"+save_dir+"_"+str(round_index)+".json", "w") as fp:
