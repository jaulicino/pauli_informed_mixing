...
percent_grid = np.linspace(1,1.7,10)
from qiskit import Aer
errors = {"Mix2":[], "Mix3":[]}
for perc in tqdm(range(len(percent_grid))):
    percent_ = percent_grid[perc] 
    print("CURRENT PERCENT: ", percent_)
    exact_pds = np.genfromtxt("exact_pds.txt")
    errors_perc = {"Mix2":[], "Mix3":[]}
    for round_index in tqdm(range(n_rounds)): # run several experiments for each method
        mix_qwc_shots = int(percent_ * 1024)
        if(round_index == 0): print("current shots : ", mix_qwc_shots)
        from qiskit import Aer
        qc = circuit(theta_test)
        if(round_index == 0):print("Pure QWC with ", len(total_qwc_bases))
        ...
        if(round_index == 0): print("Submitting Job With ", len(circuits), " circuits and ", shots_, " shots per circuit")
        counts = []
        ...
        errors_perc["Mix3"] += [est_es[3] - exact_pds[3]]
        errors_perc["Mix2"] += [est_es[2] - exact_pds[2]]

        with open("mix_expectations/"+str(round_index)+".json","w") as fp:
            json.dump(mix_expectations, fp)

        
    errors["Mix3"] += [errors_perc["Mix3"]]
    errors["Mix2"] += [errors_perc["Mix2"]]
